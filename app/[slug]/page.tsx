          ...
          <section className="bg-[#f5f9fc] px-3 py-10 sm:px-4 lg:px-4">
            <div className="mx-auto grid max-w-[1500px] gap-8">
              {bankAccounts.map((account, accountIndex) => {
                ...
                return (
                  <article key={`${account.bank}-${accountIndex}`} className="rounded-[28px] border border-[#d6e1ec] bg-white p-5 shadow-[0_24px_70px_rgba(10,58,85,0.12)] sm:p-7 lg:p-9">
                    <div className="flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-between">
                      <div className="flex min-w-0 flex-col gap-5 sm:flex-row sm:items-center sm:gap-8">
                        {logoUrl && (
                          <img src={logoUrl} alt={account.bank || "Banka logosu"} loading="lazy" decoding="async" className="h-20 w-20 shrink-0 rounded-[20px] border border-[#d6e1ec] bg-white object-contain p-3 shadow-[0_14px_35px_rgba(10,58,85,0.08)] sm:h-24 sm:w-24" />
                        )}
                        <div className="hidden h-16 w-px bg-[#d6e1ec] sm:block" />
                        <div className="min-w-0 flex-1">
                          <h2 className="text-2xl font-black leading-tight text-[#061b55] sm:text-3xl lg:text-4xl">{account.bank || "Banka Bilgisi"}</h2>
                    ...
                    <div className="mt-7 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
                      {ibans.map((iban, ibanIndex) => (
                        <div key={`${account.bank}-${iban.label}-${ibanIndex}`} className="flex min-w-0 flex-col rounded-[22px] border border-[#d6e1ec] bg-white px-5 py-5 shadow-[inset_0_1px_0_rgba(255,255,255,0.8)]">
                          <div className="flex items-center gap-4">
                            <span className="grid h-14 w-14 shrink-0 place-items-center rounded-full bg-[#061b55] text-white shadow-[0_0_0_8px_#eef3f7]">
                              <CurrencyIcon label={iban.label} />
                            </span>
                            <div className="min-w-0 flex-1">
                              <p className="text-lg font-black tracking-wide text-[#061b55]">{ibanCurrencyLabel(iban.label, ibans.length === 1)}</p>
                              <span className="mt-2 block h-px w-full bg-[#d6e1ec]" />
                            </div>
                          </div>
                          <p className="mt-5 min-w-0 break-all font-mono text-[17px] font-black leading-7 tracking-wide text-[#06122e] sm:text-lg">{iban.iban}</p>
                          <div className="mt-5">
                            <CopyIbanButton value={iban.iban} label="IBAN Kopyala" className="min-h-11 w-full justify-center rounded-[14px] bg-gradient-to-br from-[#1977dc] to-[#0747ad] px-5 text-sm shadow-[0_14px_24px_rgba(25,119,220,0.18)] hover:from-hayat-green hover:to-hayat-green" />
                          </div>
                        </div>
                      ))}
                    </div>
